...
window.onload = ()=> {
	...
	html += `<div class="art" style="background-color:${colors.white}">${getAll(artID++)}</div>`;
	html += `<div class="art" style="background-color:${colors.dark}">${getAll(artID++)}</div>`;
	html += `<div class="art" style="background-color:${colors.dark}">${getAll(artID++,colors.dark,colors.white)}</div>`;
	html += `<div class="art" style="background-color:${colors.white}">${getAll(artID++,colors.white,colors.dark)}</div>`;
	html += `<div class="art" style="background-color:${colors.black}">${getAll(artID++,-1,-1,0)}</div>`;
	html += `<div class="art" style="background-color:${colors.black}">${getAll(artID++,-1,colors.white)}</div>`;
...
const getAll = (num, fill=-1, stroke=-1, width=-1)=> '<svg viewbox="0 0 1608 1608"><g transform="translate(400 835)">' + changeArt(removeSVGElement(art.litwick),num,fill,stroke,width) + '</g><g transform="translate(120 80)">' + changeArt(removeSVGElement(art.lampent),num,fill,stroke,width) + '</g><g transform="translate(700 120)">' + changeArt(removeSVGElement(art.chandelure),num,fill,stroke,width) + '</g></svg>';